...
class BeaconFeatures(ComputeFeatureBase):
    ...
    def mark_beacons(self, streams, stream_name, user_id, day):
        ...
        if stream_name in streams:
            ...
            stream = self.CC.get_stream(
                beacon_stream_id, user_id=user_id, day=day)
    ...
    def merge_work_beacons(self, streams, stream1_name, stream2_name,
        ...
        if stream1_name in streams:
            ...
            work1_stream = self.CC.get_stream(
                beacon_stream_id1, user_id=user_id, day=day)
        ...
        if stream2_name in streams:
            ...
            work2_stream = self.CC.get_stream(
                beacon_stream_id2, user_id=user_id, day=day)
    ...
    def store_data(self, filepath, input_streams, user_id, data, str_sufix):
        ...
        output_stream_id = str(uuid.uuid3(
            uuid.NAMESPACE_DNS, str(filepath + user_id + str_sufix)))
        cur_dir = os.path.dirname(os.path.abspath(__file__))
        newfilepath = os.path.join(cur_dir, filepath)
        with open(newfilepath, "r") as f:
            metadata = f.read()
            metadata = json.loads(metadata)
            metadata["execution_context"]["processing_module"][
                "input_streams"]  = input_streams
            metadata["identifier"] = str(output_stream_id)
            metadata["owner"] = str(user_id)

            self.store(identifier=output_stream_id, owner=user_id,
                       name=metadata["name"],
